...
export class HomeService {
  ...
  getUserLoginCount(): Observable<ResponseEntity> {
    ...
    let url=`${this.api_url}`;
    let ret = this.http.get(url)
      .map((response: Response) => response.json().home_UserLoginCount as ResponseEntity)
      .catch(this.handleError);
    //return ret.delay(3000);
    return ret;
  }
  ...
  getSubjectProgressInfo():Observable<ResponseEntity>{
    ...
    let url=`${this.api_url}`;
    let ret=this.http.get(url)
        .map((response:Response)=>response.json().subject as ResponseEntity)
        .catch(this.handleError);
        return ret;
  }
  ...
  getDeviceRunInfo():Observable<ResponseEntity>{
    ...
    let url=`${this.api_url}`;
    let ret=this.http.get(url)
        .map((response:Response)=>response.json().device as ResponseEntity)
        .catch(this.handleError);
        return ret;
  }
  ...
  getDeviceBaseRunInfo():Observable<ResponseEntity>{
     ...
     let url=`${this.api_url}`;
    let ret=this.http.get(url)
        .map((response:Response)=>response.json().deviceBase as ResponseEntity)
        .catch(this.handleError);
        return ret;
  }
